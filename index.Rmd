...
1. Load the iris dataset from the `iris.tsv` file
2. Add a new column called "Individual", numbering each individual by a number.
3. Manipulate the table to lengthens data, increasing the number of rows and decreasing the number of columns.
4. Separate the Feature column into two new columns: "Part" and "Metric".
...
.pull-left[
> Need some help ?
> The package needed for that are: `readr`, `dplyr` and `tidyr`.
...
```{r ggplot18,echo=F, fig.height=3, fig.width=3, message=FALSE, warning=FALSE,dev.args = list(bg = "transparent"),fig.align="center",dpi=150,fig.retina=3}
...
p_timer <- ggplot(minutes,aes(x = x, y = y)) +
  geom_bar(data=filter(minutes,y==my.time),stat="identity", width=1, col="black",fill="#e74c3c") +
  geom_point(data = minutes,aes(x=1.5)) +
  geom_point(data = hours, aes(x = 1.5, y = y,col=y),
             size = 5, show.legend = FALSE) +
  geom_text(aes(y = 30, x = 0.5, label = my.time.str), vjust = 1, size = 8,col="#e74c3c") +
...
.pull-right[
```{r theme_display2,fig.height=3,fig.width=4}
